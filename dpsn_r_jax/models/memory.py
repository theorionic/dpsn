...
class ContentAwareRouterHead(nn.Module):
    """Content-Aware Router Head for semantic pool organization.

    This module learns to route similar content to similar regions of the pool.
    It produces region offsets that are added to the indexer's coordinates,
    guiding the model to organize data semantically across the pool grid.

    Key insight: Instead of forcing exploration through noise/loss, the router
    head actively learns WHERE different types of content should go based on
    the semantic content of the hidden states.

    Architecture:
        1. Attention-pooled query from hidden states (same pattern as LearnedIndexer)
        2. Small MLP trunk for feature extraction
        3. Two outputs: (region_r_offset, region_c_offset) in range [-scale, +scale]
        4. Learned alpha weight controls influence of router vs indexer

    Args:
        hidden_dim: Dimension of input hidden states (D).
        router_hidden_dim: Width of the MLP trunk. Default: hidden_dim // 2.
        region_scale: Maximum offset as fraction of grid (e.g., 0.1 = ±10% of grid).
                      This determines how much the router can shift coordinates.
        num_heads: Number of indexer heads (H). The router produces offsets
                   split into row_heads and col_heads: first H//2 for rows, rest for cols.
    """

    hidden_dim: int
    router_hidden_dim: int = 0  # 0 = use hidden_dim // 2
    region_scale: float = 0.1   # ±10% of grid
    num_heads: int = 1

    @nn.compact
    def __call__(self, hidden_states: jnp.ndarray) -> tuple:
        """
        Args:
            hidden_states: (B, T, D) – full encoded sequence from controller.

        Returns:
            region_r: (B, heads_per_dim) – row offsets for row-heads.
                      Applied to mu[:, :heads_per_dim].
            region_c: (B, heads_per_dim) – col offsets for col-heads.
                      Applied to mu[:, heads_per_dim:].
                      Values are in [-region_scale, +region_scale].
        """
        D = hidden_states.shape[-1]
        mlp_dim = self.router_hidden_dim if self.router_hidden_dim > 0 else D // 2
        heads_per_dim = max(1, self.num_heads // 2)

        # ── 1. Attention-pooled query ────────────────────────────────────────
        # Same pattern as LearnedIndexer: learn which positions matter for routing.
        attn_logits = nn.Dense(1, use_bias=False, name="attn_score")(hidden_states)  # (B, T, 1)
        attn_weights = jax.nn.softmax(attn_logits, axis=1)                            # (B, T, 1)
        pooled = jnp.sum(attn_weights * hidden_states, axis=1)                       # (B, D)

        # ── 2. Feature extraction trunk ──────────────────────────────────────
        # Small MLP to extract semantic routing features.
        x = nn.Dense(mlp_dim, name="fc1")(pooled)
        x = nn.gelu(x)
        x = nn.Dense(mlp_dim // 2, name="fc2")(x)
        x = nn.gelu(x)

        # ── 3. Predict region offsets ─────────────────────────────────────────
        # Output: heads_per_dim for rows + heads_per_dim for cols
        # Using tanh to bound outputs to [-1, 1], then scale to [-region_scale, +region_scale]
        raw_offsets = nn.Dense(heads_per_dim * 2, name="offset_proj")(x)  # (B, heads_per_dim * 2)
        offsets = jnp.tanh(raw_offsets) * self.region_scale                # (B, heads_per_dim * 2)

        # Split into row and column offsets matching the indexer head assignment
        region_r = offsets[:, :heads_per_dim]  # (B, heads_per_dim) - offsets for row heads
        region_c = offsets[:, heads_per_dim:]  # (B, heads_per_dim) - offsets for col heads

        return region_r, region_c
